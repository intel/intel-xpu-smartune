    ...
    return [
      {
        ...
        items: [
          ...
          { label: 'GuC FW', value: staticInfo.driver.guc_fw?.map(f => `${f.firmware} ${f.version}`).join(', ') || 'N/A' },
          { label: 'HuC FW', value: staticInfo.driver.huc_fw?.map(f => `${f.firmware} ${f.version}`).join(', ') || 'N/A' },
