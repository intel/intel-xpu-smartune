...
def _build_process_scope_snapshot(
    ...
    for cgroup_id, cg_pids in sorted(by_cgroup.items()):
        ...
        if limited_pids or limited_cgroups:
            # Limits belong to cgroups, not PIDs. A process can restart within the
            # same scope after the limit lands, and it must retain the Limited status
            # even though it is absent from the original PID snapshot.
            is_limited = (
                cgroup_id in limited_cgroups
                or any(pid in limited_pids for pid in scope_pids)
            )
